...
const SETTING_DESCRIPTION_TEXT: Record<string, SettingDescriptionText> = {
  ...
  vi: {
    ...
    concurrency: "Số lệnh gọi API cấp khóa học chạy song song thủ công. Mặc định là 1 để ổn định với nhà cung cấp miễn phí hoặc giới hạn tốc độ.",
  ...
  de: {
    ...
    concurrency: "Manuelle Parallelität für API-Aufrufe auf Kursebene. Standard ist 1 für Stabilität bei kostenlosen oder rate-limitierten Anbietern.",
  ...
  fr: {
    ...
    concurrency: "Concurrence manuelle pour les appels API au niveau du cours. La valeur par défaut est 1 pour rester stable avec les fournisseurs gratuits ou limités.",
  ...
  it: {
    ...
    concurrency: "Concorrenza manuale per le chiamate API a livello di corso. Il valore predefinito è 1 per stabilità con provider gratuiti o limitati.",
  ...
  pt: {
    ...
    concurrency: "Concorrência manual para chamadas de API no nível do curso. O padrão é 1 para estabilidade em provedores gratuitos ou com limite de taxa.",
  ...
  sv: {
    ...
    concurrency: "Manuell concurrency för API-anrop på kursnivå. Standard är 1 för stabilitet hos gratis eller rate-limited leverantörer.",
  ...
  pl: {
    ...
    concurrency: "Ręczna concurrency dla wywołań API na poziomie kursu. Domyślnie 1 dla stabilności u darmowych lub limitowanych dostawców.",
  ...
  tr: {
    ...
    concurrency: "Ders düzeyi API çağrıları için manuel concurrency. Ücretsiz veya hız sınırlı sağlayıcılarda kararlılık için varsayılan 1'dir.",
